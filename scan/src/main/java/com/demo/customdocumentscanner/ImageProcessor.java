...
import com.demo.customdocumentscanner.helpers.Quadrilateral;
import com.demo.customdocumentscanner.helpers.ScannedDocument;
import com.demo.customdocumentscanner.helpers.Utils;
    ...
    private void enhanceDocument(Mat src) {
        if (colorMode && filterMode) {
            ...
            if (filterMode) {
                ...
                Double d = scalar.val[2];
                if (d < 130) {
                    src.convertTo(src, -1, 1.8, -80);
                } else if (d > 130 && d < 150) {
                    src.convertTo(src, -1, 1.8, -145);
                } else if (d > 150 && d < 170) {
                    src.convertTo(src, -1, 1.8, -160);
                } else if (d > 170) {
                    src.convertTo(src, -1, 1.7, -170);
                }
            }
        }
    }
